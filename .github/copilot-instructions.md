...
**What this repo is:** the shared Kotlin/Spring Boot backend service for the Community Accommodation (CAS) products — Approved
Premises (CAS1), Short-Term Accommodation (CAS2) and Temporary Accommodation
(CAS3). It exposes REST APIs consumed by the CAS user interfaces, backed by
...
**CAS2** (`cas2`, `cas2v2`, `cas2hdc`), **CAS3** (Temporary Accommodation,
...
* **Stack:** Kotlin (JDK 25 — see `.sdkmanrc`), Spring Boot, Spring Data JPA,
  PostgreSQL + Flyway, WebFlux, Gradle (`./gradlew`).
* **Package layout (ADR 0029):** `casx` (`cas1`, `cas2`, `cas2v2`, `cas2hdc`,
...
* Auditing fields are explicit (`val createdAt: OffsetDateTime`,
  `var lastUpdatedAt: OffsetDateTime?`) and set by application logic — there is no
  `@CreationTimestamp`/`@UpdateTimestamp`. Soft delete via a nullable
  `deletedAt`/`isActive` filtered in queries.
...
* New, service-specific entities live under `<service>/jpa/entity/**` and are
